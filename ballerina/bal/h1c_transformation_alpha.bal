    ...
    resource function transform(http:Caller caller, http:Request req) {
        ...
        if (payload is json) {
            ...
            if (xmlPayload is xml) {
                ...
                clinetreq.setXmlPayload(sanitizeAndReturnUntainted(xmlPayload));
                ...
                http:Response res = new;
                res.statusCode = 400;
                res.setPayload( <string>xmlPayload.detail()["message"]);
                var result = caller->respond(res);
            }
...
function sanitizeAndReturnUntainted(xml input) returns @untainted xml {
